...
def display_feature_importance(model, feature_names):
    if hasattr(model, "coef_"):
        ...
        for index, (feature, importance) in enumerate(sorted_features, start=1):
            formatted_feature = ' '.join(word.capitalize() for word in feature.split('_'))
            print(f"{index}) {formatted_feature}: {importance:.3f}")
    else:
        print("The model does not support feature importances directly.")
...
def preprocess_data(df):
    ...
    df[important_features] = scaler.fit_transform(df[important_features])
